...
class ReporteBanco(models.AbstractModel):
    ...
    _description = 'Reporte Banco'
    
    def lineas(self, datos):
        lineas = []
        for linea in self.sudo().env['account.move.line'].search(
                [('account_id', '=', datos['cuenta_bancaria_id'][0]), ('date', '>=', datos['fecha_desde']),
                 ('date', '<=', datos['fecha_hasta']),('company_id','=',datos['company_id'][0])], order='date'):

            concept = False
            if linea.name:
                concept = self.sudo().env['account.payment'].search([('ref','=',linea.name),('journal_id','=',linea.move_id.journal_id.id)], limit=1).bank_concept

            detalle = {
                'fecha': linea.date,
                'documento': linea.move_id.name if linea.move_id else '',
                'nombre': linea.partner_id.name or '',
                'concepto': concept if concept else ((linea.ref if linea.ref else '') + ' ' + (linea.name if linea.name else '')),
                'debito': linea.debit,
    ...
    def balance_inicial(self, datos):
        self.env.cr.execute(
            "select coalesce(sum(amount_currency), 0) as balance_moneda from account_move_line where parent_state = 'posted' and account_id = %s and date < %s",
            (datos['cuenta_bancaria_id'][0], datos['fecha_desde']))
    ...
    def get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'data': data['form'],
            'docs': docs,
            'moneda': docs[0].cuenta_bancaria_id.currency_id or company.currency_id,
            'lineas': self.lineas,
