...
class FelReportExtraContent(models.Model):
    _name = 'fel_gt.tools.report_extra_content'
    _description = 'ReportExtraContent'
    
    file_name = fields.Char(string="Nombre Documento")
    append_extra_content = fields.Binary(string="Contenido Adicional", help="Please upload file (You can upload file like jpg, png, pdf, docx, pptx, xlsx, doc, ppt, xls, txt)")
    company_id = fields.Many2one('res.company', string='Compañia', change_default=True, required=True, default=lambda self: self.env['res.company']._company_default_get('fel_gt.tools.report_extra_content'))

    _sql_constraints = [
        ('unique_model_company', 'unique (company_id)',
    ...
    def convert_img2pdf(self, values):
        ...
        if fname:
            ...
            img_file_fd, img_file_path = tempfile.mkstemp(suffix=ext, prefix='report.convert.data.')
            ...
            img2pdf_file_fd, img2pdf_file_path = tempfile.mkstemp(suffix='.pdf', prefix='report.converted.tmp.')
    ...
    def convert_txt2pdf(self, values):
        ...
        if fname:
            (fname, ext) = os.path.splitext(fname)
            txt_file_fd, txt_file_path = tempfile.mkstemp(suffix=ext, prefix='report.convert.data.')
            ...
            pdf = FPDF()
            pdf.add_page()
            pdf.add_font('UbuntuMono', '', '/usr/share/fonts/truetype/ubuntu-font-family/UbuntuMono-BI.ttf', uni=True)
            pdf.set_font('UbuntuMono', '', 12)
            for line in open(txt_file_path, 'rb').readlines():
                pdf.write(10, txt=line.decode('utf-8'))

            txt2pdf_file_fd, txtpdf_file_path = tempfile.mkstemp(suffix='.pdf', prefix='report.converted.tmp.')
    ...
    def convert_dxp2pdf(self, values):
        ...
        if fname:
            (fname, ext) = os.path.splitext(fname)
            dxp_file_fd, dxp_file_path = tempfile.mkstemp(suffix=ext, prefix='report.dxp.data.')
    ...
    def write(self, values):
        ...
        return super(FelReportExtraContent, self).write(values)
            ...
            if not file_name.endswith('.pdf'):
                if file_name.endswith('.txt') or file_name.endswith('.text'):
                    ...
                    values.update(self.convert_img2pdf(values))
                        ...
                        file_name.endswith('pptx') or file_name.endswith('ppt'):
                    values.update(self.convert_dxp2pdf(values))
        return super(FelReportExtraContent, self).create(values)
