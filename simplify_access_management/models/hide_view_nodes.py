...
class HideViewNodes(models.Model):
    ...
    def _store_btn_data(self, btn, smart_button=False, smart_button_string=False):
        string_value = self._context.get('string_value', False)
        ...
        if self.model_id and self.model_name:
            ...
            for view in view_list:
                for views in view_obj.search([('model', '=', self.model_name), ('type', '=', view)]):
                    # Odoo 19: usar get_view en lugar de fields_view_get
                    try:
                    ...
                    for btn in object_link:
                        if (btn.text and '\n' not in btn.text
                            and 'type' in btn.attrib.keys() and btn.attrib['type']
                            and 'name' in btn.attrib.keys() and btn.attrib['name']):
                            domain = [
                                ('button_type', '=', btn.get('type')),
...
class StoreModelNodes(models.Model):
    ...
    @api.depends('attribute_string', 'attribute_name', 'is_smart_button', 'node_option')
