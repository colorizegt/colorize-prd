...
class IrUiView(models.Model):
                ...
                for hide_field in hide_fields:
                    for field_id in hide_field.field_id:
                        if ((node.tag == 'field' and node.get('name') == field_id.name)
                            or (node.tag == 'label' and 'for' in node.attrib.keys()
    ...
    def _postprocess_tag_button(self, node, name_manager, node_info):
        postprocessor = getattr(super(), '_postprocess_tag_button', False)
        if postprocessor:
            super()._postprocess_tag_button(node, name_manager, node_info)
    ...
    def _postprocess_tag_page(self, node, name_manager, node_info):
        postprocessor = getattr(super(), '_postprocess_tag_page', False)
        if postprocessor:
            super()._postprocess_tag_page(node, name_manager, node_info)
    ...
    def _postprocess_tag_a(self, node, name_manager, node_info):
        postprocessor = getattr(super(), '_postprocess_tag_a', False)
        if postprocessor:
            super()._postprocess_tag_a(node, name_manager, node_info)
    ...
    def _postprocess_tag_div(self, node, name_manager, node_info):
        postprocessor = getattr(super(), '_postprocess_tag_div', False)
        if postprocessor:
            super()._postprocess_tag_div(node, name_manager, node_info)
    ...
    def _postprocess_tag_filter(self, node, name_manager, node_info):
        postprocessor = getattr(super(), '_postprocess_tag_filter', False)
        if postprocessor:
            super()._postprocess_tag_filter(node, name_manager, node_info)
    ...
    def _postprocess_tag_label(self, node, name_manager, node_info):
        postprocessor = getattr(super(), '_postprocess_tag_label', False)
        hide_field_obj = self.env['hide.field'].sudo()
        if postprocessor:
